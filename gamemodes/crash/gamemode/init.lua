	...
	if not self.NaggedAboutScore and timeleft < 60 and timeleft > 50 and GAMEMODE:InRound() then
		...
		if losingteam != -1 then
			for k, pl in pairs(team.GetPlayers(losingteam)) do
				pl:ChatPrint("Your team is losing badly, start breaking stuff!")
			end
		end
...
function GM:EntityTakeDamage( ent, dmginfo )
	...
	if attacker:IsPlayer() then
		...
		if ent:IsPlayer() and ent:Team() != attacker:Team() and IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetClass() == "weapon_crowbar"
			...
			if table.HasValue(propClasses,ent:GetClass()) and (attacker:Team() == ent:TeamSide() or dmginfo:IsBulletDamage()) then
				...
				if (attacker:Team() == ent:TeamSide()) then
					...
					if (attacker.stupiditywarning == 5) then
						evolve:Notify( attacker, evolve.colors.red, "This is your own crate; defend it, not hit it." )
					end
					if (attacker.stupiditywarning == 10) then
						evolve:Notify( attacker, evolve.colors.red, "This is your own crate; defend it, not hit it." )
					end
					if (attacker.stupiditywarning == 15) then
						evolve:Notify( attacker, evolve.colors.red, "Please read the F4 menu if you don't know how to play." )
					end
				end
...
function GM:PlayerDeath( pl, inflictor, attacker ) 
	...
	if IsValid(attacker) and attacker:IsPlayer() then
		...
		if IsValid(attacker:GetActiveWeapon())
...
function GM:PropBreak( attacker, prop ) 
	...
	if attacker:IsPlayer() then 
		...
		if IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetClass() == "weapon_crowbar" 
