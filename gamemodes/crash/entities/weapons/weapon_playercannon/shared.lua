...
if ( CLIENT ) then
	...
	killicon.AddFont("weapon_playercannon", SWEP.IconFont, SWEP.IconLetter, Color(1, 177, 236, 255 ))

	function SWEP:DrawWeaponSelection( x, y, wide, tall, alpha )
		draw.SimpleText( "b", "HalfLife2", x + wide/2, y + tall/2, Color( 1, 177, 236 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
...
function SWEP:PrimaryAttack()
	...
	if IsValid(self:GetAbsorbedPlayer()) then
...
function SWEP:FirePlayer( force, was_forced )
		...
		if CLIENT then
			if force < 100 and self.lastfailshotnag < CurTime() - 10 then
				if not was_forced then
					evolve:Notify( ply, evolve.colors.red, "HOLD your fire button to launch with more force!" )
				else
					evolve:Notify( ply, evolve.colors.red, "Don't hold players longer than 10 seconds!" )
				end
	...
	if SERVER then
		...
		self.dt.Absorbed = Entity(0)
		self.Owner:SetAbsorbedPlayer( Entity(0) )
	end
...
function SWEP:Think()
	...
	if self.dt.ChargeInit then
		...
		if IsValid(self:GetAbsorbedPlayer()) and self.dt.AbsorbStartTime < CurTime()-10 then
...
function SWEP:SecondaryAttack()
	...
	if not IsFirstTimePredicted() then return end
	if IsValid(self:GetAbsorbedPlayer()) then return end
